---
name: skill-manager
description: Manage the skills framework — validate, usage stats, archive unused skills (Sunset Protocol), restore archived skills, improve skills, and update the CHANGELOG. Use when managing, auditing, or maintaining the skills framework.
...
Meta-skill for managing the skills framework. Subcommands: `validate`, `usage`, `list`, `improve`, `archive`, `restore`, `changelog`.
...
# $SKILLS_REPO is set by skills-env.sh (generated by setup.sh) to the repo's install location
```
